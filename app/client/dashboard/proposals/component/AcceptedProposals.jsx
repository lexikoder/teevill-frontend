...
export default function AcceptedProposals({ payments }) {
              ...
              <Tr key={index}>
                <Td>
                  <Flex align="center" gap={3}>
                    <Avatar
                      size="sm"
                      name={payment?.submittedBy?.firstName + " " + payment?.submittedBy?.lastName }
                      src={payment?.submittedBy?.profileImage}
                    />
                    <Box>
                      <Text fontWeight="bold">{payment?.submittedBy?.firstName + " " + payment?.submittedBy?.lastName}</Text>
                ...
                <Td px="30px">{payment.job?.title}</Td>
                <Td>{dayjs(payment.createdAt).format("DD-MM-YYYY")}</Td>

                 <Td>{formatToNaira(payment?.job?.budget) || ""}</Td>
                 <Td>{payment.timeLine?.charAt(0)?.toUpperCase() +""+ payment?.timeLine?.slice(1)+" "+ "months"|| ""}</Td>
